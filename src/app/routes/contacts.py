...
async def read_contacts(
    skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
...
async def create_contact(body: ContactBase, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    try: 
        new_contact = ContactCreate(user_id=current_user.id, **body.model_dump(exclude_unset=True))
...
async def update_note(
    body: ContactBase, contact_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    contact_controller = ContactsController(db)
    contact = await contact_controller.update_contact(user_id = current_user.id, id=contact_id,contact = body)
    if contact is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found"
        )
...
async def remove_conctact(contact_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    contact_controller = ContactsController(db)
    contact = await contact_controller.delete_contact(user_id=current_user.id, id=contact_id)
    if contact is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found"
        )
...
async def search_contacts(
    name: str = None, surname: str = None, email: str = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    contact_controller = ContactsController(db)
    contact = await contact_controller.search_contact(user_id=current_user.id,name=name, surname=surname, email=email)
    return contact
